...
def validate_channel_config(channel_config_df):
    mfc_gas_ids = channel_config_df.apply(
        lambda x: "{}_{}_{}".format(x["Channel"], x["Gas"], x["Rank"]), axis=1
    )
    unique_ids = np.unique(mfc_gas_ids)

    if len(mfc_gas_ids) != len(unique_ids):
        raise ValueError(
            "Error in MFC config file: if multiple MFCs feed the same gas to the same channel, "
...
def split_flow(tot_flow, ranks, ranges):
    ...
    if np.isscalar(tot_flow):
        ...
        if tot_flow > np.sum(ranges):
            raise ValueError(
                "Total requested flow ({} SCCM) exceeds total range of MFCs ({} SCCM)".format(
...
def get_mfc_flows(channel_config, mfc_row, channel_flow, gas_frac):
    ...
    try:
        dup_rows = channel_config[
            channel_config["Channel_Gas"] == mfc_row["Channel_Gas"]
        ]
        if len(dup_rows) == 1:
            # No redundant MFCs
        ...
        raise err.__class__(
            "Error encountered while determining flow for MFC {}:\n{}".format(
                mfc_row["DisplayName"], err
            )
        )


def generate_mfc_schedule(
    channel_config_file, exp_schedule_file, reduction_config_file, destination_dir
):
    # Read files
    ...
    channel_config["Full_ID"] = channel_config.apply(
        lambda x: "{}_{}".format(x["Port"], x["ID"]), axis=1
    )
    channel_config["Channel_Gas"] = channel_config.apply(
        lambda x: "{}_{}".format(x["Channel"], x["Gas"]), axis=1
    )
    ...
    for i, row in channel_config.iterrows():
        ...
        channel = row[
            "Channel"
        ]  # get_mfc_property(channel_config, 'Channel', row['ID'], row['Port'])
        gas = row[
            "Gas"
        ]  # get_mfc_property(channel_config, 'Gas', row['ID'], row['Port'])
        ...
        # Get MFC flow
        flow, index_over_range = get_mfc_flows(
            channel_config, row, channel_flow, gas_frac
        )

        # Validate flows
        if len(index_over_range[0]) > 0:
            raise ValueError(
                "Requested flow for MFC {} exceeds MFC range at the following steps: {}".format(
        ...
        if reduction_config_file is not None:
            ...
            # Get MFC flow
            red_flow, index_over_range = get_mfc_flows(
                channel_config, row, red_channel_flow, red_frac
            )

            # Validate flows
            if len(index_over_range[0]) > 0:
                raise ValueError(
                    "Requested flow for MFC {} exceeds MFC range at the following REDUCTION steps: {}".format(
                        display_name,
                        red_schedule["ReductionStep"].values[index_over_range],
                    )
                )
    ...
    mfc_schedule.to_csv(
        os.path.join(destination_dir, "OUT_mfc_flow_schedule.csv"), index_label="Index"
    )
    if red_schedule is not None:
        red_schedule.to_csv(
            os.path.join(destination_dir, "OUT_reduction_flow_schedule.csv"),
            index_label="Index",
        )
...
def generate_cmd_kwargs(exp_schedule_file, meas_config_file, destination):
    ...
    def generate_cmd(row):
        ...
        if config_sheet is not None:
            ...
            simple_args = config_options[
                ~config_options["ArgType"].isin(["store_true", "store_false"])
            ]
            ...
            arg_string = " ".join(
                [
                    get_simple_arg_string(ar["ArgName"], ar["ArgType"], ar["ArgValue"])
                    for i, ar in simple_args.iterrows()
                ]
            )
            # Handle action arguments
            action_args = config_options[
                config_options["ArgType"].isin(["store_true", "store_false"])
            ]

            def get_action_arg_string(arg_name, action, arg_value):
                if action == "store_true" and (
                    str(arg_value).upper() == "TRUE" or arg_value == 1
                ):
                    string = f"--{arg_name}"
                elif action == "store_false" and (
                    str(arg_value).upper() == "FALSE" or arg_value == 0
                ):
                    string = f"--{arg_name}"
                else:
                    string = ""
            ...
            arg_string += " " + " ".join(
                [
                    get_action_arg_string(ar["ArgName"], ar["ArgType"], ar["ArgValue"])
        ...
        if row["StepType"] in ("Measure", "Reduction"):
            ...
            raise ValueError(
                "Invalid step type {} at step {} in file {}".format(
                    row["StepType"], row["Step"], exp_schedule_file
                )
            )
...
def get_bubbler_temp_and_status(ph2o):
    ...
    bub_temp[active_index] = np.interp(
        ph2o[active_index], ph2o_values, ph2o_temp_range, left=-1, right=120
    )
    ...
    if len(low_index[0]) > 0:
        raise Exception(
            "Requested pH2O values {} are too low at indices {}. Allowed pH2O range: {:.4f}, {:.4f}".format(
                ph2o[low_index], low_index[0], np.min(ph2o_values), np.max(ph2o_values)
            )
        )
    if len(high_index[0]) > 0:
        raise Exception(
            "Requested pH2O values {} are too high at indices {}. Allowed pH2O range: {:.4f}, {:.4f}".format(
                ph2o[high_index],
                high_index[0],
                np.min(ph2o_values),
                np.max(ph2o_values),
...
def generate_tempctrl_schedule(
    channel_config_file, exp_schedule_file, tempctrl_config_file, destination
):
    # Read files
    channel_config = pd.read_csv(channel_config_file)
    ...
    for i, row in tc_config.iterrows():
        ...
        if row["Type"] == "Furnace":
            ...
            if ph2o_col not in exp_schedule.columns:
                raise Exception(
                    f"No pH2O specified for channel {channel} in exp_schedule_file"
                )

            # Get bubbler temperature corresponding to specified pH2O
            bub_temps, bub_status = get_bubbler_temp_and_status(
                exp_schedule[ph2o_col].values
            )
            ...
            raise Exception(
                "Invalid temperature controller type {} for {} in temperature controller "
...
def get_mfc_property(channel_config, property_name, mfc_id, port=None):
    if property_name not in channel_config.columns:
        raise ValueError(
            f"Invalid property_name {property_name}. Property must be defined in channel_config. "
    ...
    if port is None:
        ...
        return dict(
            zip(channel_config["ID"].values, channel_config[property_name].values)
        ).get(mfc_id, None)
    else:
        mfc_df = channel_config[
            (channel_config["ID"] == mfc_id) & (channel_config["Port"] == port)
        ]
        if len(mfc_df) == 1:
            ...
            raise ValueError(
                f"Multiple entries found for mfc_id {mfc_id} and port {port}"
            )
...
def check_channel_gases(exp_schedule, channel_config):
    ...
    for channel in channels:
        ...
        if not is_subset(sched_gases, channel_gases):
            raise ValueError(
                f"Gases for channel {channel} are inconsistent in channel configuration "
    ...
    for channel in channels:
        ...
        if len(frac_cols) > 0:
            ...
            if not check_equality(np.round(total_frac, 5), np.ones(len(total_frac))):
                ...
                raise ValueError(
                    f"Gas fractions in channel schedule do not sum to 1 for channel {channel} "
...
def get_tempctrl_property(tempctrl_config, property_name, tempctrl_id, port=None):
    if property_name not in tempctrl_config.columns:
        raise ValueError(
            f"Invalid property_name {property_name}. Property must be defined in tempctrl_config. "
    ...
    if port is None:
        ...
        return dict(
            zip(tempctrl_config["ID"].values, tempctrl_config[property_name].values)
        ).get(tempctrl_id, None)
    else:
        mfc_df = tempctrl_config[
            (tempctrl_config["ID"] == tempctrl_id) & (tempctrl_config["Port"] == port)
        ]
        if len(mfc_df) == 1:
            return mfc_df[property_name].values[0]
        elif len(mfc_df) == 0:
            ...
            raise ValueError(
                f"Multiple entries found for mfc_id {tempctrl_id} and port {port}"
            )
...
def check_tempctrl_config(exp_schedule, channel_config, tempctrl_config):
    ...
    if num_furnace != 1:
        raise Exception(
            "There must be one and only one temperature controller with Type=Furnace in the temperature "
    ...
    for channel in channels:
        ...
        if ph2o_col in columns:
            if np.max(exp_schedule[ph2o_col]) > 0:
                ...
                bub_df = tempctrl_config[
                    (tempctrl_config["Channel"].isin([channel, "All"]))
                    & (tempctrl_config["Type"] == "Bubbler")
                ]
                if len(bub_df) == 0:
                    raise Exception(
                        f"No bubbler assigned to channel {channel} in temp controller config file"
                    )
                elif len(bub_df) > 1:
                    raise Exception(
                        f"Multiple bubblers assigned to channel {channel} in temp controller config file"
                    )
