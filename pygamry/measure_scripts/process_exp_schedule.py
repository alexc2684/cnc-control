...
from file_proc import (
    generate_mfc_schedule,
    generate_cmd_kwargs,
    generate_tempctrl_schedule,
    get_mfc_property,
)
import argparse
import easygui
...
parser = argparse.ArgumentParser(
    description="Generate equipment schedule files for experiment control"
)
...
try:
    ...
    mfc_schedule, red_schedule = generate_mfc_schedule(
        args.channel_config_file,
        args.experiment_schedule_file,
    ...
    tc_schedule = generate_tempctrl_schedule(
        args.channel_config_file,
        args.experiment_schedule_file,
        args.tempctrl_config_file,
        tc_schedule_file,
    )
    ...
    generate_cmd_kwargs(
        args.experiment_schedule_file, args.measurement_config_file, cmd_arg_file
    )
    ...
    def plot_flow_schedule(input_schedule_df, output_flow_df, channel_config):
        ...
        start_indices = np.concatenate(
            (input_schedule_df.index, [len(input_schedule_df)])
        )
        ...
        for channel, axrow in zip(channels, axes):
            # Row 0: gas fractions
            frac_columns = [
                col for col in sched_columns if col.find(f"{channel}Frac") == 0
            ]
            for frac_col in frac_columns:
                plot_values = np.zeros(len(plot_index))
                for st, val in zip(
                    start_indices[:-1], input_schedule_df[frac_col].values
                ):
            ...
            plot_values = np.zeros(len(plot_index))
            for st, val in zip(
                start_indices[:-1], input_schedule_df[f"{channel}Flow"].values
            ):
                plot_values[plot_index >= st] = val
    ...
    def plot_temp_schedule(output_temp_df, tempctrl_config):
        ...
        for i, row in tempctrl_config.iterrows():
            ...
            if row["Type"] == "Furnace":
                ...
                for st, val in zip(
                    start_indices[:-1], output_temp_df[status_col].values
                ):
    ...
    if num_red_steps == 0:
        ...
        if confirmation:
            red_fig = plot_flow_schedule(red_config, red_schedule, channel_config)
            red_fig_path = os.path.join(
                schedule_path, "OUT_reduction_flow_schedule.png"
            )
            red_fig.savefig(red_fig_path, dpi=350)
    ...
    easygui.msgbox(
        msg=f"The following error occurred while processing configuration files:"
