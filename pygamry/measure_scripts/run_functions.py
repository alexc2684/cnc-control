...
def run_eis(dtaq, pstat, args, file_suffix, V_oc=None, show_plot=False):
    ...
    dtaq.run(
        ...
        plot_interval=1,
        plot_type="all",
        condition_time=args.eis_condition_time,
...
def equil_ocv(pstat, v_oc, duration, suffix, exp_notes, data_path, kst_path):
    pstatic = DtaqPstatic(write_mode="interval", write_interval=1, exp_notes=exp_notes)
...
def run_pwrpol(dtaq, pstat, args, file_suffix, show_plot=False):
    ...
    if args.pwrpol_direction not in direction_options:
        raise ValueError(
            f"Invalid direction {args.pwrpol_direction}. Options: {direction_options}"
        )
    ...
    for i, direction in enumerate(directions):
        ...
        dtaq.run(
            pstat,
            args.pwrpol_i_final,
            args.pwrpol_scan_rate,
            args.pwrpol_sample_period,
            v_min=args.pwrpol_v_min,
            v_max=args.pwrpol_v_max,
            show_plot=show_plot,
            result_file=pwrpol_file,
            kst_file=kst_file,
...
def run_pstatic(
    dtaq, pstat, args, file_suffix, V_oc=None, pwrpol_dtaq=None, show_plot=False
):
    ...
    # Get cutoff current from polarization curve
    pstatic_i_min, pstatic_i_max = get_pstatic_cutoff(
        pstatic_VDC, args, V_oc, pwrpol_dtaq
    )
    ...
    dtaq.run(
        pstat,
        pstatic_VDC,
        args.pstatic_duration,
        args.pstatic_sample_period,
        i_min=pstatic_i_min,
        i_max=pstatic_i_max,
        timeout=None,
        show_plot=show_plot,
...
def run_chrono(dtaq, pstat, args, file_suffix, show_plot=False, repeats=1):
    ...
    if args.chrono_step_type == "dstep":
        ...
        dtaq.configure_triplestep_signal(
            args.chrono_s_init,
            s_rms,
            args.chrono_t_init,
            args.chrono_t_step,
    ...
    result_file = os.path.join(
        args.data_path, "CHRONO{}_{}.DTA".format(tag_letter, file_suffix)
    )
...
def run_v_sweep(dtaq, pstat, args, file_suffix, V_oc):
    ...
    if args.vsweep_direction not in direction_options:
        raise ValueError(
            f"Invalid direction {args.vsweep_direction}. Options: {direction_options}"
        )
    ...
    iv_df.to_csv(
        os.path.join(args.data_path, "VSWEEPIV_{}.DTA".format(file_suffix)),
        sep="\t",
        index_label="Pt",
    )
...
def test_resistance(pstat, t_step=2.5, v_step=0.01):
    ...
    dt_chrono.configure_dstep_signal(
        v_oc, v_oc + v_step, v_oc - v_step, 0.5, t_step, t_step, 1e-3
    )
...
def run_hybrid(sequencer, pstat, args, file_suffix, show_plot=False, **kw):
    ...
    if sequencer.chrono_mode != "galv":
        raise ValueError(
            "run_hybrid expects a sequencer with chrono_mode = 'galv';"
    ...
    if args.hybrid_eis_mode == "pot":
        if args.hybrid_i_init != 0:
            raise ValueError(
                "Potentiostatic EIS can only be used for hybrid measurements centered at open circuit"
            )
    ...
    decimate = not args.disable_decimation
    if decimate:
        sequencer.configure_decimation(
            args.decimate_during,
            args.decimation_prestep_points,
            args.decimation_interval,
            args.decimation_factor,
    ...
    # Get EIS frequencies
    eis_freq = get_eis_frequencies(
        args.hybrid_eis_max_freq, args.hybrid_eis_min_freq, args.hybrid_eis_ppd
    )

    # Configure chrono step
    step_types = ["triple", "geo"]
    if args.hybrid_step_type == "triple":
        sequencer.configure_triple_step(
            args.hybrid_i_init,
            i_rms,
            args.hybrid_t_init,
            args.hybrid_t_step,
            args.hybrid_t_sample,
            eis_freq,
            z_guess=z_guess,
        )
    elif args.hybrid_step_type == "geo":
        sequencer.configure_geo_step(
            args.hybrid_i_init,
            i_rms,
            args.hybrid_t_init,
            args.hybrid_geo_t_short,
            args.hybrid_t_step,
            args.hybrid_t_sample,
            args.hybrid_geo_num_scales,
            args.hybrid_geo_steps_per_scale,
            eis_freq,
            end_at_init=args.hybrid_geo_end_at_init,
            end_time=args.hybrid_geo_end_time,
            z_guess=z_guess,
        )
        ...
        raise ValueError(
            f"Invalid measurement_type {args.hybrid_step_type}. Options: {step_types}"
        )
    # else:
    ...
    if args.hybrid_eis_mode == "pot":
        if v_oc is None:
            v_oc = test_ocv(pstat)
        sequencer.configure_eis(
            eis_freq, dc_amp=v_oc, ac_amp=abs(args.hybrid_v_rms), z_guess=z_guess
        )
    ...
    sequencer.run(
        ...
        kst_path=args.kst_path,
        file_suffix=file_suffix,
        rest_time=args.hybrid_rest_time,
...
def run_hybrid_staircase(sequencer, pstat, args, file_suffix, jv_data=None):
    print("Running HYBRID staircase")

    if sequencer.chrono_mode != "galv":
        raise ValueError(
            "run_hybrid_staircase expects a sequencer with chrono_mode = 'galv';"
            f" received sequencer with mode {sequencer.chrono_mode}"
        )
    ...
    if args.staircase_direction not in direction_options:
        raise ValueError(
            f"Invalid direction {args.staircase_direction}. Options: {direction_options}"
        )
    ...
    full_eis_freq = get_eis_frequencies(
        args.staircase_full_eis_max_freq,
        args.staircase_full_eis_min_freq,
    ...
    for i, direction in enumerate(directions):
        ...
        if jv_data is not None:
            sequencer.configure_staircase_from_jv(
                args.hybrid_i_init,
                v_rms,
                args.hybrid_t_init,
                args.hybrid_t_step,
                ...
                jv_data,
                eis_freq,
                args.staircase_num_steps,
                v_limits=v_limits,
                step_type=args.hybrid_step_type,
                geo_kwargs=geo_kwargs,
            )
        else:
            sequencer.configure_staircase(
                args.hybrid_i_init,
                i_rms,
                args.hybrid_t_init,
                args.hybrid_t_step,
        ...
        sequencer.run_staircase(
            ...
            full_frequencies=full_eis_freq,
...
def equilibrate_pstatic(pstat, args, file_suffix):
    ...
    dtaq = DtaqPstaticEquil(
        args.equil_window_seconds,
        slope_thresh_pct_per_minute=args.equil_slope_thresh,
        min_wait_time_minutes=args.equil_min_wait_time,
        exp_notes=args.exp_notes,
        write_mode="interval",
        write_interval=int(1 / args.equil_sample_period),
    )
    ...
    dtaq.run(
        ...
        pstatic_vdc,
        args.equil_duration,
        args.equil_sample_period,
        result_file=result_file,
        kst_file=kst_file,
        i_min=args.pequil_i_min,
        i_max=args.pequil_i_max,
    )
    print("PEQUIL done\n")
...
def equilibrate_gstatic(pstat, args, file_suffix):
    ...
    # Configure dtaq. Write once per second
    dtaq = DtaqGstaticEquil(
        args.equil_window_seconds,
        slope_thresh_mv_per_minute=args.equil_slope_thresh,
        min_wait_time_minutes=args.equil_min_wait_time,
        exp_notes=args.exp_notes,
        write_mode="interval",
        write_interval=int(1 / args.equil_sample_period),
    ...
    dtaq.run(
        ...
        result_file=result_file,
        kst_file=kst_file,
        v_min=args.gequil_v_min,
        v_max=args.gequil_v_max,
    )
    print("GEQUIL done\n")
