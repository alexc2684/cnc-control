...
class DtaqReadZ(GamryDtaqEventSink):
    ...
    def set_mode(self, mode: str):
        ...
        if self.user_ac_ierange is not None:
            ...
            if self.user_ac_ierange != rec_ac_ierange:
                warnings.warn(
                    f"For {mode} mode, the recommended value "
    ...
    def set_ac_ierange(self, ac_ierange: bool | None):
        ...
        if ac_ierange is not None:
            ...
            if ac_ierange != recommended:
                warnings.warn(
                    f"For {self.mode} mode, the recommended value "
        ...
        if self.ac_ierange:
            ...
            if self.mode == "galv":
                ...
                IERange = self.pstat.TestIERangeAC(
                    self.ac_amp_req, v_ac_max, self.dc_amp_req, s_dc_max, frequency
                )
            else:
                i_ac_max = 2 * self.ac_amp_req / z_guess
                IERange = self.pstat.TestIERangeAC(
                    i_ac_max, self.ac_amp_req, s_dc_max, self.dc_amp_req, frequency
                )
        else:
            ...
            if self.mode == "galv":
                ...
                i_max = (
                    2
                    * (abs(self.dc_amp_req) + (2**0.5) * self.ac_amp_req)
                    / self.z_guess
                )
    ...
    def _IGamryReadZEvents_OnDataDone(self, this, gc_readzstatus):
        ...
        if (
            self.gc_readzstatus == GamryCOM.ReadZStatusOk
            and self.dtaq.StatusMessage() in retry_reasons[self.gc_readzspeed]
        ):
        ...
        if (
            self.gc_readzstatus == GamryCOM.ReadZStatusRetry
            and self.passes < self.max_passes
        ):
            ...
            print(
                "Retry at frequency {:.2e} Hz for reason: {}".format(
                    frequency, self.dtaq.StatusMessage()
                )
            )
            if self.mode == "galv":
            ...
            self.dtaq.Measure(
                frequency, self.ac_amp_req
            )  # self.measure_point would reset passes
        else:
            ...
            if self.gc_readzstatus == GamryCOM.ReadZStatusOk:
                ...
                # print('Gain:', self.dtaq.Gain())
                # Store data
                self.z_data[self.frequency_index] = [
                    data_timedelta
                ] + self.get_current_zdata()
            else:
                # Measurement failed
                if self.passes == self.max_passes:
                    print(
                        "Measurement at {:.2e} Hz unsuccessful after {} passes".format(
                    ...
                    print(
                        "Measurement at {:.2e} Hz failed with reason: {}".format(
    ...
    def initialize_pstat(self):
        ...
        if self.mode == "galv":
            self.pstat.SetCASpeed(
                3
            )  # Switched from 2 to 3 as recommended by Abe Krebs 1/30/23
    ...
    def initialize_figure(self):
        ...
        if self.plot_type in ("all", "nyquist"):
            ...
            lax_n = LiveNyquist(
                nyquist_ax
            )  # , axis_extend_ratio={'x': 0.25, 'y': 0.25})
            ...
            # Create artist
            lax_n.add_line_artist(
                "z", update_nyquist, marker=".", ms=4, alpha=0.6, ls=""
            )
        ...
        if self.plot_type in ("all", "bode"):
            ...
            xlim = (
                np.min(self.frequencies) / 5,
                np.max(self.frequencies) * 5,
            )  # passed to LiveAxes below
            for ax in bode_axes:
                ax.set_xscale("log")
            ...
            # Create artists
            lax_b1.add_line_artist(
                "modulus", update_modulus, marker=".", ms=4, alpha=0.6, ls=""
            )
            lax_b2.add_line_artist(
                "phase", update_phase, marker=".", ms=4, alpha=0.6, ls=""
            )
        ...
        def update_status(frame):
            if self.frequency_index < len(self.frequencies):
                text = "Measuring Z at {:.2e} Hz".format(
                    self.frequencies[self.frequency_index]
                )
            else:
        ...
        if self.plot_type in ("all", "nyquist"):
            ...
            axis_limits = get_nyquist_limits(
                nyquist_ax, np.array([self.z_guess * 0.1, self.z_guess * (0.9 - 0.5j)])
            )
    ...
    def get_dataframe_to_write(self, start_index, end_index):
        ...
        """
        return pd.DataFrame(
            self.z_data[start_index:end_index],
    ...
    def get_kst_dataframe(self, start_index, end_index):
        ...
        """
        try:
            kst_data = self.z_data[start_index:end_index, self.kst_column_index].astype(
                float
            )
        except TypeError:
        ...
        if destination_file is not None:
            if is_final_write:
                ...
                if self.write_mode == "interval":
                    ...
                    data_string = self.generate_data_string(
                        data_func, 0, self.total_points, indent
                    )
                    with open(destination_file, "a") as f:
                        f.write(data_string)
