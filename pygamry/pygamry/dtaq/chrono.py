...
class DtaqChrono(GamryDtaqEventSink):
    ...
    def initialize_pstat(self):
        ...
        if self.start_with_cell_off:
            if self.mode == "pot":
                ...
                self.pstat.SetVoltage(
                    0
                )  # Set voltage to 0, corresponding to zero current:
    ...
    def configure_mstep_signal(
        self, s_init, s_stepsize, t_init, t_step, t_sample, n_steps
    ):
    ...
    def configure_dstep_signal(
        self, s_init, s_step1, s_step2, t_init, t_step1, t_step2, t_sample
    ):
        self.signal = client.CreateObject("GamryCOM.GamrySignalDstep")

        # Round durations to sample interval
        t_init, t_step1, t_step2 = round_to_sample_interval(
            t_sample, [t_init, t_step1, t_step2]
        )
    ...
    def configure_triplestep_signal(self, s_init, s_rms, t_init, t_step, t_sample):
        ...
        times, signal, step_times = signals.make_triplestep_signal(
            s_init, s_rms, t_init, t_step, t_sample
        )
    ...
    def configure_geostep_signal(
        ...
        # Round durations to sample interval
        t_init, t_short, t_long = round_to_sample_interval(
            t_sample, [t_init, t_short, t_long]
        )

        # Build the signal array
        times, signal, step_times = signals.make_geostep_signal(
            s_init,
            s_final,
            s_min,
            s_max,
    ...
    def configure_decimation(
        ...
        if decimate_during not in ("write", "run"):
            raise ValueError(
                f"Invalid decimate_during {decimate_during}. Options: run, write"
            )
        elif decimate_during == "write" and self.write_mode != "once":
            raise ValueError(
                "Decimation during write is only available when write_mode=once"
            )
        elif decimate_during == "write" and decimation_factor is None:
            raise ValueError(
                "Decimation factor must be specified when decimate_during=write"
            )
        elif decimate_during == "run" and self.signal_params.get("t_sample", 1) < 1e-4:
            warnings.warn(
                "Runtime decimation is not functional when sampling period is shorter than 1e-4 s"
            )
    ...
    def get_step_times(self, include_erroneous: bool = True):
        ...
        if self.signal_params["signal_class"] == "Dstep":
            ...
            step_times = (
                np.array([0, self.signal_params["t_step1"]])
                + 0.99 * self.signal_params["t_sample"]
            )
        elif self.signal_params["signal_class"] == "Mstep":
            step_times = (
                self.signal_params["t_init"]
                + np.arange(0, self.signal_params["n_steps"])
                * self.signal_params["t_step"]
            )
        ...
        if include_erroneous:
            ...
            meas_step_times = self.data_array[
                meas_step_index, self.cook_columns.index("Time")
            ]
    ...
    def run(
        ...
        **run_kw,
        ...
        if decimate and self.decimate_during is None:
            ...
            decimate
            and self.decimate_during == "run"
            and self.signal_params.get("t_sample", 1) < 1e-4
        ):
            warnings.warn(
                "Runtime decimation is not functional when sampling period is shorter than 1e-4 s"
            )
        ...
        if i_max is not None and self.mode == "galv":
            ...
            warnings.warn(
                "For best results in potentiostatic mode, expected max current should be supplied to i_max"
            )

        if self.signal_params["t_sample"] < 1e-2 and self.write_mode != "once":
            warnings.warn(
                "When chrono sample period is short, write_mode should be set to 'once' "
        ...
        if self.signal_params["signal_class"] == "Dstep":
            add_info += [
                [
                    "TPRESTEP",
                    "QUANT",
                    "{:.4e}".format(self.signal_params["t_init"]),
                    "Pre-step Delay Time (s)",
                ],
                [
                    f"{s_label}PRESTEP",
                    "QUANT",
                    "{:.4e}".format(self.signal_params["s_init"]),
                    "Pre-step Delay Time (s)",
                ],
                [
                    "TSTEP1",
                    "QUANT",
                    "{:.4e}".format(self.signal_params["t_step1"]),
                    "Step 1 Time (s)",
                ],
                [
                    f"{s_label}STEP1",
                    "QUANT",
                    "{:.4e}".format(self.signal_params["s_step1"]),
                    "Step 1 value",
                ],
                [
                    "TSTEP2",
                    "QUANT",
                    "{:.4e}".format(self.signal_params["t_step2"]),
                    "Step 2 Time (s)",
                ],
                [
                    f"{s_label}STEP2",
                    "QUANT",
                    "{:.4e}".format(self.signal_params["s_step2"]),
                    "Step 2 value",
                ],
            ]
        elif self.signal_params["signal_class"] == "Mstep":
            add_info += [
                ...
                [
                    "TSTEP",
                    "QUANT",
                    "{:.4e}".format(self.signal_params["t_step"]),
                    "Step Time (s)",
                ],
                [
                    f"{s_label}STEP",
                    "QUANT",
                    "{:.4e}".format(self.signal_params["s_stepsize"]),
                    "Step size",
                ],
            ]
    ...
    def decimate_index(self):
        ...
        decimate_index = get_decimation_index(
            times, step_times, self.signal_params["t_sample"], *self.decimate_args
        )
    ...
    def write_to_file(
        ...
        if (
            ...
            and is_final_write
            and self.decimate
            and self.decimate_during == "write"
            and self.write_mode == "once"
        ):
        ...
        for end_time in np.concatenate([self.get_step_times(), [times[-1] + 1e-10]]):
            if window is None:
                ...
                window_index = np.where(
                    (times >= start_time + step_duration * 0.95) & (times < end_time)
                )
    ...
    def _get_cook_values(self, name):
        try:
            ...
            raise ValueError(
                f"Invalid value name {name}. Must be one of dtaq cook columns"
            )
