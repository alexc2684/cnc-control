...
class HybridSequencer:
    def __init__(
        self, chrono_mode="galv", eis_mode="galv", update_step_size=True, exp_notes=None
    ):
    ...
    def configure_triple_step(
        self, s_init, s_rms, t_init, t_step, t_sample, frequencies, z_guess=None
    ):
    ...
    def configure_hzh_step(
        ...
        self.mstep_args.append(
            [s_init + s_half_step, s_half_step, t_init, t_step, t_sample, 1]
        )
    ...
    def configure_staircase(
        ...
        if step_type not in step_options:
            raise ValueError(
                f"Invalid step_type {step_type} for staircase. Options: {step_options}"
            )
        ...
        if step_type == "gzh":
            if geo_kwargs is None:
                raise ValueError(
                    "geo_kwargs must be provided for staircase step_type gzh"
                )
            else:
    ...
    def configure_staircase_from_jv(
        self,
        i_init,
        v_rms_target,
        t_init,
        ...
        if jv_data is not None:
            if type(jv_data) == str:
                ...
                raise ValueError(
                    f"Expected jv_data to be a path or DataFrame; got type {type(jv_data)} instead"
                )
        ...
        if len(i_sort) < 2:
            ...
            high_fit = np.polyfit(
                i_sort[-extrap_points:], v_sort[-extrap_points:], deg=1
            )
            ...
            jv_data_extrap = pd.DataFrame(
                np.vstack([i_extrap, v_extrap]).T, columns=["Im", "Vf"]
            )
    ...
    def configure_decimation(
        ...
        self.dt_chrono.configure_decimation(
            decimate_during,
            prestep_points,
            decimation_interval,
            decimation_factor,
    ...
    def get_next_i_rms(self, i_init, v_rms_target):
        ...
        if self.staircase_jv_data is not None:
            ...
            i_next = pol.estimate_next_i(
                self.staircase_i_hist,
                self.staircase_v_hist,
                penalty_vec=[0, 0, 0.1],
                v_step=v_rms_target * 2 * np.sqrt(2),
                deg=2,
                num_points=3,
                prev_step_prior=1e-2,
                i_offset=None,
                v_offset=0,
                i_lambda=0,
            )
    ...
    def run(
        ...
        if decimate and self.dt_chrono.decimate_args is None:
            raise RuntimeError(
                "Decimation must be configured prior to calling run if decimate=True"
            )
        ...
        def run_eis():
            ...
            self.dt_eis.run(
                ...
                *self.eis_args,
                ...
                kst_file=eis_kst_file,
                show_plot=show_plot,
                max_passes=eis_max_passes,
            )
        ...
        try:
            ...
            if self.measurement_type in ["triple_step", "geo_step"]:
                ...
                if data_path is not None:
                    eis_file = os.path.join(
                        data_path, "{}_{}.DTA".format(eis_tag, file_suffix)
                    )
                    chrono_file = os.path.join(
                        data_path, "{}_{}.DTA".format(chrono_tag, file_suffix)
                    )
                else:
                    eis_file = None
                    chrono_file = None
                ...
                def run_chrono():
                    ...
                    if self.measurement_type == "triple_step":
                        ...
                        self.dt_chrono.configure_triplestep_signal(
                            *self.triplestep_args
                        )
                    else:
                    ...
                    self.dt_chrono.run(
                        pstat,
                        decimate=decimate,
                        result_file=chrono_file,
                        kst_file=chrono_kst_file,
                        show_plot=show_plot,
                        filter_response=False,
                    )
                    ...
                    if filter_response and decimate:
                        ...
                        self.dt_chrono.result_file = chrono_file.replace(
                            ".DTA", "_Filtered.DTA"
                        )
                ...
                if data_path is not None:
                    ...
                    chrono_file_pre = os.path.join(
                        data_path, "{}_{}-a.DTA".format(chrono_tag, file_suffix)
                    )
                    chrono_file_post = os.path.join(
                        data_path, "{}_{}-b.DTA".format(chrono_tag, file_suffix)
                    )
                else:
                    eis_file = None
                ...
                def run_chrono_a():
                    ...
                    if filter_response and decimate:
                        self.dt_chrono.filter_response = True
                        self.dt_chrono.result_file = chrono_file_pre.replace(
                            ".DTA", "_Filtered.DTA"
                        )
                        self.dt_chrono.kst_file = None
    ...
    def run_staircase(
        ...
        equil_time=0,
        rest_time=0,
        run_full_eis_pre=False,
        run_full_eis_post=False,
        full_frequencies=None,
        start_with_cell_off=True,
        leave_cell_on=False,
        filter_response=False,
        show_plot=False,
        ...
        if self.eis_mode == "pot":
            raise ValueError(
                "Hybrid staircase is not implemented for potentiostatic EIS"
            )

        if self.staircase_args is None:
            raise RuntimeError(
                "Measurement must be configured before calling run_staircase"
            )
        ...
        s_init, s_rms, t_init, t_step, t_sample, frequencies, z_guess = (
            self.staircase_args
        )
        # Chrono step is sqrt(2) * s_rms
        s_half_step = s_rms * np.sqrt(2)
        ...
        try:
            if run_full_eis_pre:
                ...
                eis_file = os.path.join(
                    data_path, "{}_{}_Pre.DTA".format(eis_tag, file_suffix)
                )
                eis_kst_file = os.path.join(kst_path, "Kst_EIS.DTA")
                self.dt_eis.run(
                    pstat,
            ...
            for step in range(self.staircase_num_steps):
                ...
                if self.staircase_step_type in ["hzh", "fz", "fhzh"]:
                    getattr(self, f"configure_{self.staircase_step_type}_step")(
                        s_init, s_rms, t_init, t_step, t_sample, frequencies, z_guess
                    )
                ...
                #     self.configure_fhzh_step(s_init, s_rms, t_init, t_step, t_sample, frequencies, z_guess)
                    ...
                    raise ValueError(
                        f"Invalid step_type {self.staircase_step_type} for run_staircase"
                    )
                ...
                if self.chrono_mode == "galv" and self.update_step_size:
                    ...
                    if v_rms_target is None:
                        v_rms_target = (self.meas_v_max - self.meas_v_min) / (
                            2 * np.sqrt(2)
                        )
                    ...
                    s_half_step = s_rms * np.sqrt(2)
                    print(
                        "Last v_full_step: {:.3f} mV".format(
                            1000 * (self.meas_v_max - self.meas_v_min)
                        )
                    )
                ...
                if self.meas_v_min <= self.staircase_v_limits[0]:
                    print(
                        "Lower voltage limit reached: {:.3f} (measured) <= {:.3f} (threshold)".format(
                    ...
                    print(
                        "Upper voltage limit reached: {:.3f} (measured) >= {:.3f} (threshold)".format(
            ...
            if run_full_eis_post:
                ...
                eis_file = os.path.join(
                    data_path, "{}_{}_Post.DTA".format(eis_tag, file_suffix)
                )
                eis_kst_file = os.path.join(kst_path, "Kst_EIS.DTA")
                self.dt_eis.run(
                    pstat,
