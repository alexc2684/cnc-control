...
class GamryDtaqEventSink(object):
    ...
    def PumpEvents(self, timeout):
        ...
        self.handler_routine = ctypes.WINFUNCTYPE(ctypes.c_int, ctypes.c_uint)(
            handler_routine
        )
        ...
        try:
            ...
            for rep in range(repeats):
                ...
                if repeats > 1 and result_file is not None:
                    ...
                    current_file = "{}_{}{}".format(
                        result_file[:ext_index], rep, result_file[ext_index:]
                    )
                else:
    ...
    def initialize_figure(self):
        self.fig, self.axes = plt.subplots(
            1, len(self.axes_def), figsize=(len(self.axes_def) * 4.5, 3.25)
        )
    ...
    def dataframe(self):
        ...
        if data_columns is None:
            data_columns = list(
                np.arange(self.data_array.shape[1], dtype=int).astype(str)
            )
        ...
        # Construct DataFrame
        return pd.DataFrame(
            self.data_array[:, keep_index[0]],
            columns=np.array(data_columns)[keep_index],
        )
    ...
    def get_dataframe_to_write(self, start_index, end_index):
        ...
        try:
            write_data = self.data_array[
                start_index:end_index, self.column_index_to_write
            ].astype(float)
        except TypeError:
            write_data = self.data_array[
                start_index:end_index, self.column_index_to_write
    ...
    def get_kst_dataframe(self, start_index, end_index):
        ...
        """
        try:
            kst_data = self.data_array[
                start_index:end_index, self.kst_column_index
            ].astype(float)
        except TypeError:
    ...
    def format_start_date_time(self):
        ...
        if self.write_precision is not None:
            time_dec_str = str(rel_round(time_dec, self.write_precision))[
                2 : 2 + self.write_precision
            ]
        else:
            time_dec_str = str(time_dec)
        time_string = time.strftime(
            "{}.{}".format(time_format_code.split(" ")[1], time_dec_str), gmtime
        )
    ...
    def generate_header_text(self):
        ...
        text = (
            ...
            + self.get_date_time_text()
            + self.get_dtaq_header()
            + self.get_notes_text()
    ...
    def write_to_files(self, new_count, is_final_write):
        ...
        self._last_write_index = self.write_to_file(
            self.get_dataframe_to_write,
            self.result_file,
            self._active_file,
            self._last_write_index,
            new_count,
            is_final_write,
            indent=True,
        )

        self._last_kst_write_index = self.write_to_file(
            self.get_kst_dataframe,
            self.kst_file,
            None,
            self._last_kst_write_index,
            new_count,
            is_final_write,
            indent=False,
        )
    ...
    def get_file_offset_info(self, append_to_file, result_file):
        if result_file is not None:
            ...
            self.file_time_offset = (
                self.start_time - self.file_start_time - self.data_time_offset
            )
