...
class EquilWrapper(object):
    def __init__(
        self,
        equil_window_seconds,
        slope_thresh,
        breakin_window_seconds=2,
        min_wait_time_minutes=None,
        filter_func=None,
        **init_kw,
    ):
    ...
    def check_equilibration_status(self):
        ...
        if elapsed_minutes >= self.min_wait_time_minutes:
            ...
            if self.num_points - self.breakin_window_points >= self.equil_window_points:
                ...
                status = signal_is_stable(
                    t_window, s_window, self.slope_thresh, filter_func=self.filter_func
                )
    ...
    def _IGamryDtaqEvents_OnDataAvailable(self, this):
        ...
        self.is_equilibrated = (
            np.sum(self.equil_status_history) == self.equil_status_history.maxlen
        )
    ...
    def run(
        ...
        t_sample,
        max_iter=1,
        require_consecutive_statuses=10,
        **kw,
    ):
        ...
        self.breakin_window_points = int(
            np.ceil(self.breakin_window_seconds / t_sample)
        )
        ...
        while not self.is_equilibrated:
            ...
            super().run(
                pstat, s_const, duration, t_sample, append_to_file=append_to_file, **kw
            )
...
class DtaqPstaticEquil(EquilWrapper, DtaqPstatic):
    def __init__(
        self,
        equil_window_seconds,
        slope_thresh_pct_per_minute=0.5,
        breakin_window_seconds=2,
        min_wait_time_minutes=None,
        **init_kw,
    ):
        super().__init__(
            equil_window_seconds,
            slope_thresh_pct_per_minute,
            breakin_window_seconds,
    ...
    def get_slope_data(self):
        ...
        i_deno = np.sign(i_med) * max(
            abs(i_med), 6 * np.std(i_window)
        )  # Minimum normalization current
...
class DtaqGstaticEquil(EquilWrapper, DtaqGstatic):
    def __init__(
        self,
        equil_window_seconds,
        slope_thresh_mv_per_minute=0.5,
        breakin_window_seconds=2,
        min_wait_time_minutes=None,
        **init_kw,
    ):
