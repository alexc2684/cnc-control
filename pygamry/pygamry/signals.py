...
def check_signal_size(signal):
    ...
    if len(signal) > max_signal_size:
        raise ValueError(
            f"Length of signal array ({len(signal)}) exceeds maximum length ({max_signal_size})"
        )
...
def make_geostep_signal(
    ...
    if s_max <= s_min:
        raise ValueError(
            f"s_max must be greater than s_min. Received s_min={s_min}, s_max={s_max}"
        )

    if t_short >= t_long:
        raise ValueError(
            "t_short must be less than t_long, but received t_short={:.3e} "
    ...
    if num_scales < 2:
        raise ValueError(
            "Geostep requires at least 2 duration scales (num_scales); "
            f"received num_scales={num_scales}"
        )

    if steps_per_scale < 1 or steps_per_scale > 3:
        raise ValueError(
            f"steps_per_scale must be between 1 and 3; received {steps_per_scale}"
        )
    ...
    step_durations = np.array(
        [round(sd / t_sample, 0) * t_sample for sd in step_durations]
    )
    ...
    if (
        abs((s_final - s_min) - (s_max - s_final)) < s_range * flex_thresh
        and init_sign != 0
    ):
        # Flexible last step
        final_sign = 0
    ...
    duration = (
        t_init
        + steps_per_scale * np.sum(step_durations)
        + int(stutter) * step_durations[0]
    )
    times = np.arange(0, duration + 1e-8, t_sample)
    check_signal_size(times)
    ...
    if end_at_init and s_final != s_init:
        ...
        end_times = np.arange(
            times[-1] + t_sample, times[-1] + end_time + 1e-8, t_sample
        )
